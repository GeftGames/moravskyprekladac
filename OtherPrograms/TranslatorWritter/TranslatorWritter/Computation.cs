    ...
    internal static class Computation {
        ...
        public static bool FindTableInHTML(string html, string tableName, ref List<Table> tables) {
            ...
            while (_html.Contains("<table class=\""+tableName+"\"")) {
                ...
                int tableStart = _html.IndexOf("<table class=\""+tableName+"\" style=\"text-align: center\">")+("<table class=\""+tableName+"\" style=\"text-align: center\">").Length;
        ...
        public static Row GetRowByFirstCellText(Table table, string text) { 
            foreach (Row row in table.Rows) { 
                if (row.Cells.Count>0) {
                    if (row.Cells[0].Text.ToLower()==text) return row;
                }
            }
            return null;
        }
